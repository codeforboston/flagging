...
def process_data(
    ...
    # 10 minute increment.
    df_hobolink["_sig_rain"] = (df_hobolink["rain"].rolling(24 * 6).max()) >= SIGNIFICANT_RAIN
    ...
    df["sum_rain_48h_to_96h"] = df["rain"].rolling(96).sum() - df["rain"].rolling(48).sum()
    df["sum_rain_0h_to_168h"] = df["rain"].rolling(168).sum()
